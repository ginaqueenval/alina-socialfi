...
import { getLocalCartCount } from "../pages/localCartState.js";

const FC26_MENU = [
  { to: "/fc26/coins", key: "header.nav.fc26Menu.coins" },
  { to: "/fc26/players", key: "header.nav.fc26Menu.players" },
  { to: "/fc26/sbc", key: "header.nav.fc26Menu.sbc" },
  { to: "/fc26/packs", key: "header.nav.fc26Menu.packs" },
];
...
export default function AppHeader({ isLoggedIn, playerDisplayName, onRequestLogout }) {
  ...
  const [fc26MenuOpen, setFc26MenuOpen] = useState(false);
  ...
  const syncCartCount = useCallback(async () => {
    ...
    try {
      ...
      setCartCount(Number(res.data || 0) + localCount);
    } catch {
  ...
  const renderMenuContent = (isMobileMenu) => (
    <>
      ...
      {(isMobileMenu
        ? [
            ...ACCOUNT_LINKS,
            { to: "/fc26", key: "header.nav.fc26" },
            ...FC26_MENU.map((link) => ({ ...link, sub: true })),
            ...NAV_LINKS,
          ]
        ...
        <Link
          ...
          className={`text-sm transition-colors ${
            l.to === "/cart"
              ...
              : isMobileMenu
                ? l.sub
                  ? "mx-3 flex min-h-[40px] items-center justify-center border-t border-white/8 px-4 text-center text-[13px] text-[#9AA7BD] transition-colors hover:bg-white/5 hover:text-[#E7EDF7]"
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-30 border-b border-white/8 bg-[#070A0F]/84 shadow-[0_14px_34px_rgba(0,0,0,0.22)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav className="hidden items-center gap-6 text-sm text-[#9AA7BD] md:flex">
          <div
            className="relative"
            onMouseEnter={() => setFc26MenuOpen(true)}
            onMouseLeave={() => setFc26MenuOpen(false)}
          >
            <Link
              to="/fc26"
              className="inline-flex items-center gap-1.5 py-2 hover:text-[#E7EDF7]"
              aria-haspopup="menu"
              aria-expanded={fc26MenuOpen}
            >
              {t("header.nav.fc26")}
              <svg
                xmlns="http://www.w3.org/2000/svg"
                ...
                className={`h-3.5 w-3.5 transition-transform duration-200 ${
                  fc26MenuOpen ? "rotate-180 text-[#E7EDF7]" : ""
                }`}
                aria-hidden="true"
              >
                <path
            ...
            {fc26MenuOpen && (
              <div className="absolute left-0 top-full z-50 pt-2">
                <div className="w-48 overflow-hidden rounded-[22px] border border-white/10 bg-[#0B1220] shadow-[0_16px_34px_rgba(0,0,0,0.34)]">
                  {FC26_MENU.map((l) => (
                    <Link
                      key={l.to}
                      to={l.to}
                      onClick={() => setFc26MenuOpen(false)}
                      className="flex w-full items-center px-4 py-3 text-sm font-medium text-[#9AA7BD] transition-colors hover:bg-white/5 hover:text-[#E7EDF7]"
                    >
                      {t(l.key)}
                    </Link>
                  ))}
                  ...
                  <Link
                    to="/fc26"
                    onClick={() => setFc26MenuOpen(false)}
                    className="flex w-full items-center px-4 py-3 text-sm font-semibold text-[#00FF9A] transition-colors hover:bg-[#00FF9A]/10"
                  >
                    {t("header.nav.fc26Menu.all")}
                  </Link>
                </div>
              </div>
            )}
